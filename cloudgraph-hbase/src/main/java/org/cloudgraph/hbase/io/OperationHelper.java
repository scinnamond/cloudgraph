...
public class OperationHelper {
    ...
    public OperationHelper(FederatedOperation federatedOperation) {
	...
	public TableWriter createTableWriter(PlasmaType type) 		
    {
    	...
    	TableWriter tableContext = new GraphTableWriter(
    		tableConfig, federatedOperation);
    		...
    		HTableInterface con) throws IOException
    {
		...
		if (changeSummary.isCreated(dataObject)) {
    		...
    		graphState = new GraphState(this.federatedOperation.getMarshallingContext());
        }
		else {
    		if (result.isEmpty())
            ...
            graphState = new GraphState(Bytes.toString(state), 
            		this.federatedOperation.getMarshallingContext());
    	}   		
    	return graphState;
    }
}
