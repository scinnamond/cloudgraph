...
public class ImportExportConfigDataBinding implements DataBinding {

    private static Log log = LogFactory.getLog(ImportExportConfigDataBinding.class);
    public static String FILENAME_SCHEMA_CHAIN_ROOT = "ImportExportConfig.xsd";

    // just classes in the same package where can find the above respective
    // schema files via Class.getResource*
    public static Class<?> RESOURCE_CLASS = ImportExportConfigDataBinding.class;

    private ValidatingUnmarshaler validatingUnmarshaler;

    public static Class<?>[] FACTORIES = { org.cloudgraph.web.config.imex.ObjectFactory.class, };

    @SuppressWarnings("unused")
    private ImportExportConfigDataBinding() {
    }

    public ImportExportConfigDataBinding(BindingValidationEventHandler validationEventHandler)
